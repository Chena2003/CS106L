...
bool operator==(const HashMap<K, M, H>& lhs, const HashMap<K, M, H>& rhs) {
    ...
    for(const auto& item : lhs) {
        auto [pre, curr] = rhs.find_node(item.first);
        if(curr == nullptr || curr->value.second != item.second)
            return false;
        // if(!rhs.contains(item.first) || rhs.at(item.first) != item.second)
        //     return false;
    }
    return true;
}
...
HashMap<K, M, H>::HashMap(HashMap<K, M, H>&& map) :
    ...
    map._buckets_array.resize(map._buckets_array.size(), nullptr);
}
